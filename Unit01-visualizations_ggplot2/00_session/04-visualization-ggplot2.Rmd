...
Or to color each period in the plot differently, you could use a vector as an input to the argument **color**. **`ggplot2`** will provide a different color corresponding to different values in the vector. Here is an example where we color with **`period`**:

```{r color-by-decoration-type-1, purl=FALSE}
ggplot(data = surveys_complete, mapping = aes(x = diameter, y = length)) +
...
We can use boxplots to visualize the distribution of diameter within each period:
...
The `theme` call angles the x-axis labels to 45 degrees so they don't overlap: we will learn more about `theme` later in the lesson.
...
>
> So far, we've looked at the distribution of diameter within periods. Try making a new plot to explore the distribution of another variable within each period.
>
...
Let's calculate number of counts per year for each decoration_type. First we need to group the data and count records within each group:
...
Unfortunately, this does not work because we plotted data for all the decoration types together. We need to tell ggplot to draw a line for each decoration_type by modifying the aesthetic function to include `group = decoration_type`:

```{r time-series-by-decoration-type, purl = FALSE}
ggplot(data = yearly_counts, aes(x = year, y = n, group = decoration_type)) +
    geom_line()
```

We will be able to distinguish decoration types in the plot if we add colors (using `color` also automatically groups the data):
...
`ggplot` has a special technique called *faceting* that allows the user to split one plot into multiple plots based on a factor included in the dataset. We will use it to make a time series plot for each decoration_type:
...
Now we would like to split the line in each plot by the diagnostic of each individual measured. To do that we need to make counts in the data frame grouped by `year`, `decoration_type`, and `diagnostic`:

```{r, purl = FALSE}
 yearly_diagnostic_counts <- surveys_complete %>%
                      count(year, decoration_type, diagnostic)
```

We can now make the faceted plot by splitting further by diagnostic using `color` (within a single plot):
...
```{r facet-by-decoration-type-and-diagnostic-white-bg, purl=FALSE}
 ggplot(data = yearly_diagnostic_counts,
        mapping = aes(x = year, y = n, color = diagnostic)) +
     geom_line() +
...
> ### Challenge
>
> Use what you just learned to create a plot that depicts how the average diameter of each period changes through the years.
>
...
```{r number-decoration-type-year-with-right-labels, purl = FALSE}
ggplot(data = yearly_diagnostic_counts, aes(x = year, y = n, color = diagnostic)) +
    geom_line() +
    facet_wrap(vars(decoration_type)) +
    labs(title = "Observed  through time",
...
```{r number-decoration-type-year-with-right-labels-xfont-size, purl=FALSE}
ggplot(data = yearly_diagnostic_counts, mapping = aes(x = year, y = n, color = diagnostic)) +
    geom_line() +
    facet_wrap(vars(decoration_type)) +
    labs(title = "Observed decoration types through time",
        x = "Year of observation",
        y = "Number of artefacts") +
    theme_bw() +
...
```{r number-decoration-type-year-with-right-labels-xfont-orientation, purl = FALSE}
...
> -   Try using a different color palette (see [https://www.cookbook-r.com/Graphs/Colors\_(ggplot2)/](https://www.cookbook-r.com/Graphs/Colors_(ggplot2)/){.uri}).
...
plot_diameter <- ggplot(data = surveys_complete, aes(x = period, y = diameter)) +
  geom_boxplot() +
  labs(x = "Period", y = expression(log[10](diameter))) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  scale_y_log10()
